...
public class MainController implements Initializable {
    ...
    @FXML TableColumn<ExtractorTask, Exception> statusColumn;
    @FXML TableView<ExtractorTask> taskView;
    @FXML TableColumn<ExtractorTask, String> fileColumn;
    @FXML TableColumn<ExtractorTask, Double> progressColumn;
    @FXML MenuItem mnScanNow;
    @FXML MenuItem mnDelete;
    @FXML Button startButton;
    @FXML MenuItem mnClear;
    @FXML TextField filterText;
    @FXML MenuItem mnDeleteFile;
    @FXML MenuItem mnToggleFilter;
    @FXML BorderPane borderPanel;
    @FXML BorderPane rootPane;
    @FXML Spinner<Integer> threadSpinner;
    ...
    private FilteredList<ExtractorTask> filteredTask =
            ...
            for (ExtractorTask task : selectedItems) {
                ...
                if (!taskList.contains(task)) {
                    try {
                        ...
                        DialogBuilder.exception(e).withMessage(
                                Messages.getMessage("error.delete",task.getFileName()))
                        .show();
                    }
                }
            }
        ...
        try {
            ...
            .ifPresent(response -> {
                ...
                try {
                    ...
                    DialogBuilder.exception(ce)
                    .withHeader(Messages.getMessage("error.saveConfig"))
                    .show();
                }
            });
    ...
    private void loadConfigModel() {
        try {
            ...
            DialogBuilder.warn()
            .withHeader(Messages.getMessage("error.loadConfigTitle"))
            .withMessage(Messages.getMessage("error.loadConfigMessage"))
            .show();
        }
    }
    ...
    private void initializeTaskView() {
        ...
        statusColumn.setCellValueFactory(new PropertyValueFactory<ExtractorTask, Exception>("exception"));
        ...
        fileColumn.prefWidthProperty().bind(taskView.widthProperty()
                .subtract(statusColumn.getWidth()+progressColumn.getWidth()+30.0));
        ...
        startButton.textProperty().bind(new StringBinding() {
            ...
            protected String computeValue() {
                return Messages.getMessage(
                        taskManager.isRunning() ? "main.stop.label" : "main.start.label");
            }
        });
